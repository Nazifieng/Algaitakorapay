...
@bot.message_handler(commands=['kora'])
def check_kora_status(msg):
    ...
    try:
        ...
        bot.send_message(uid, "🔍 Checking KoraPay status...")

        r = requests.get(
            f"{KORA_BASE}/merchant/profile",
            headers=headers,
            timeout=20
        )
        ...
        # RAW DEBUG
        try:
            debug_text = f"KORA STATUS RAW:\nStatus Code: {r.status_code}\nResponse: {r.text}"
            print(debug_text)
            bot.send_message(ADMIN_ID, f"🐞 {debug_text}")
        except:
            pass

        if r.status_code != 200:
            bot.send_message(uid, f"❌ Failed to connect to Kora\nCode: {r.status_code}")
            return

        data = r.json()

        if not data.get("status"):
            bot.send_message(uid, "❌ Kora responded but status is FALSE")
            return
        ...
        is_active = profile.get("is_active", "unknown")

        msg_text = f"""✅ KoraPay Account Info
...
"""
        ...
        bot.send_message(uid, f"❌ ERROR checking Kora:\n{e}")
...
def groupitem_deeplink_handler(msg):  
    ...
    try:
        ...
        debug(f"STEP 11: Pay URL -> {pay_url}")
    except Exception as e:
        debug(f"ERROR KORAPAY: {e}")
        cur.close()
        conn.close()
        return
